...
int MakeYesNoHorzMenu(int spacesBetween, u32 bg) {
  return MakeHorizontalMenu(YesNoEntries, ARR_LEN(YesNoEntries), spacesBetween,
                            bg, 0);
}
...
int MakeHorizontalMenu(MenuEntry_t *entries, int len, int spacesBetween, u32 bg,
  ...
  while (1) {
    for (int i = 0; i < len; i++) {
      (highlight == i) ? SETCOLOR(bg, entries[i].optionUnion)
                       : SETCOLOR(entries[i].optionUnion, bg);
