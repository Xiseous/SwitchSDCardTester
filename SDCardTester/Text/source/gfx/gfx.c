...
void gfx_clear_partial_grey(u8 color, u32 pos_x, u32 height) {
  memset(gfx_ctxt.fb + pos_x * gfx_ctxt.stride, color,
         height * 4 * gfx_ctxt.stride);
}
...
void gfx_con_init() {
  ...
  gfx_con.savedx = 0;
  gfx_con.savedy = 0;
  gfx_con.fgcol = 0xFFFFFFFF; // White text for OLED
  gfx_con.fillbg = 1;
  gfx_con.bgcol = 0xFF000000; // Black background for OLED
  gfx_con.mute = 0;
}
...
void gfx_set_rect_grey(const u8 *buf, u32 size_x, u32 size_y, u32 pos_x,
                       u32 pos_y) {
  u32 pos = 0;
...
void gfx_boxGrey_old(int x0, int y0, int x1, int y1, u8 shade){
        for (int y = y0; y <= y1; y++){
                memset(gfx_ctxt.fb + y * gfx_ctxt.stride + x0, shade, (x1 - x0)
* 4);
        }
}
*/
...
*/
void gfx_set_rect_rgb(const u8 *buf, u32 size_x, u32 size_y, u32 pos_x,
                      u32 pos_y) {
  u32 pos = 0;
  for (u32 y = pos_y; y < (pos_y + size_y); y++) {
    for (u32 x = pos_x; x < (pos_x + size_x); x++) {
      gfx_ctxt.fb[x + y * gfx_ctxt.stride] =
          buf[pos + 2] | (buf[pos + 1] << 8) | (buf[pos] << 16);
      pos += 3;
    }
  }
}

void gfx_set_rect_argb(const u32 *buf, u32 size_x, u32 size_y, u32 pos_x,
                       u32 pos_y) {
  u32 *ptr = (u32 *)buf;
  for (u32 y = pos_y; y < (pos_y + size_y); y++)
    for (u32 x = pos_x; x < (pos_x + size_x); x++)
...
void gfx_render_bmp_argb(const u32 *buf, u32 size_x, u32 size_y, u32 pos_x,
                         u32 pos_y) {
  for (u32 y = pos_y; y < (pos_y + size_y); y++) {
    for (u32 x = pos_x; x < (pos_x + size_x); x++)
      gfx_ctxt.fb[x + y * gfx_ctxt.stride] =
          buf[(size_y + pos_y - 1 - y) * size_x + x - pos_x];
  }
}
